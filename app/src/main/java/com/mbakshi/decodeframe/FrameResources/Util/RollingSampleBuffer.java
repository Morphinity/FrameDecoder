...
import com.mbakshi.decodeframe.FrameResources.Util.Allocate.Allocation;
...
public final class RollingSampleBuffer {
    ...
    private void readEncryptionData(SampleHolder sampleHolder, SampleExtrasHolder extrasHolder) {
        ...
        sampleHolder.cryptoInfo.set(subsampleCount, clearDataSizes, encryptedDataSizes,
                extrasHolder.encryptionKeyId, sampleHolder.cryptoInfo.iv, Constants.CRYPTO_MODE_AES_CTR);
    ...
    private void dropDownstreamTo(long absolutePosition) {
        int relativePosition = (int) (absolutePosition - totalBytesDropped);
        int allocationIndex = relativePosition / allocationLength;
        //Log.i("RollingBuffer", "dropDownstream " + allocationIndex);
        //Log.i("RollingBuffer", "relativePosition " + relativePosition + " allocationLength " + allocationLength + " totalBytesDropped " + totalBytesDropped);
     ...
     * @param length The maximum length of the read, or {@link Constants#LENGTH_UNBOUNDED} if the caller does
    ...
    public int appendData(DataSource dataSource, int length) throws IOException {
        ...
        length = length != Constants.LENGTH_UNBOUNDED ? Math.min(length, remainingAllocationCapacity)
        ...
        public synchronized long skipToKeyframeBefore(long timeUs) {
            ...
            while (searchIndex != relativeWriteIndex) {
                if (timesUs[searchIndex] > timeUs) {
                    // We've gone too far.
                    break;
                } else if ((flags[searchIndex] & Constants.SAMPLE_FLAG_SYNC) != 0) {
