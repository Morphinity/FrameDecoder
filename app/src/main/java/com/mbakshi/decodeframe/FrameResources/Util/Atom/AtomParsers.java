...
import com.mbakshi.decodeframe.FrameResources.Tracks.TrackEncryptionBox;
import com.mbakshi.decodeframe.FrameResources.Tracks.TrackSampleTable;
import com.mbakshi.decodeframe.FrameResources.Util.CodecUtil.Ac3Util;
import com.mbakshi.decodeframe.FrameResources.Util.CodecUtil.CodecSpecificDataUtil;
...
public final class AtomParsers {
    ...
    public static Track parseTrak(Atom.ContainerAtom trak, Atom.LeafAtom mvhd) {
        ...
        Pair<Integer, Pair<Long, Integer>> header = parseTkhd(trak.getLeafAtomOfType(Atom.TYPE_tkhd).data);
        int id = header.first;
        long duration = header.second.first;
        int rotation = header.second.second;
        Log.i("Parser", "Rotation " + rotation);
        long movieTimescale = parseMvhd(mvhd.data);
        long durationUs;
        if (duration == -1) {
            durationUs = Constants.UNKNOWN_TIME_US;
        } else {
            durationUs = Utilities.scaleLargeTimestamp(duration, Constants.MICROS_PER_SECOND, movieTimescale);
        }
        ...
        if(stsdData != null && stsdData.mediaFormat != null) {
            stsdData.mediaFormat.rotation = rotation;
        }
    ...
    public static TrackSampleTable parseStbl(Track track, Atom.ContainerAtom stblAtom) {
        ...
        for (int i = 0; i < sampleCount; i++) {
            ...
            flags[i] = stss == null ? Constants.SAMPLE_FLAG_SYNC : 0;
            if (i == nextSynchronizationSampleIndex) {
                flags[i] = Constants.SAMPLE_FLAG_SYNC;
        ...
        Utilities.scaleLargeTimestampsInPlace(timestamps, 1000000, track.timescale);
     ...
     */
    private static Pair<Integer, Pair<Long, Integer>> parseTkhd(ParsableByteArray tkhd) {
        ...
        // Get Rotation
        tkhd.skipBytes(16);
        int cosTheta = tkhd.readInt();
        int sinTheta = tkhd.readInt();

        int rotation = 0;
        if(cosTheta == 65536 && sinTheta == 0) {
            rotation = 0;
        }
        else if(cosTheta == 0 && sinTheta == 65536) {
            rotation = 90;
        }
        else if(cosTheta == -65536 && sinTheta == 0) {
            rotation = 180;
        }
        else if(cosTheta == 0 && sinTheta == -65536) {
            rotation = 270;
        ...
        return Pair.create(trackId, Pair.create(duration, rotation));
    }
    ...
    private static StsdDataHolder parseStsd(ParsableByteArray stsd, long durationUs) {
        ...
        for (int i = 0; i < numberOfEntries; i++) {
            ...
            if (childAtomType == Atom.TYPE_avc1 || childAtomType == Atom.TYPE_avc3
                ...
                holder.mediaFormat = com.mbakshi.decodeframe.FrameResources.Util.Media.MediaFormat.createTtmlFormat();
    ...
    private static void parseAvcFromParent(ParsableByteArray parent, int position, int size,
        ...
        out.mediaFormat = MediaFormat.createVideoFormat(MimeTypes.VIDEO_H264, MediaFormat.NO_VALUE,
                durationUs, width, height, 0, pixelWidthHeightRatio, initializationData);
    }
    ...
    private static MediaFormat parseMp4vFromParent(ParsableByteArray parent, int position, int size,
        ...
        return MediaFormat.createVideoFormat(
                MimeTypes.VIDEO_MP4V, MediaFormat.NO_VALUE, durationUs, width, height, 0, initializationData);
    }
